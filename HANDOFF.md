...
- ある国が**1勝するたび**、その国に点を置いた人へ他全員が点を払う**ゼロサム精算**。
  ...
  （PK勝ち含む）。**特別ルール2つ**: ①優勝（決勝に勝つ）はご褒美で**2勝分**にカウント。
  ②**3位決定戦は番外試合でノーカウント**（勝敗とも引き分け扱い。ただし試合表には表示し、
  対象外と明記）。実装は fetch_matches.py / sim/tournament.py / sim/theoretical.py と
  両HTMLの winsSoFar()・matchRow() に入っている。
